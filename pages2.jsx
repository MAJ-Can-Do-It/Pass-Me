      ...
      <section style={{
        ...
        <div className="container" style={{ position:'relative', zIndex:1 }}>
          <div style={{
            ...
            ✨ 47 tutors already earning on Pass Me
          </div>
...
function TutorRegistration() {
  ...
  const [form, setForm] = useStateP2({
    fullName:'', faculty:'', year:'', subjects:'',
          ...
          <div style={card}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
              <div style={{ gridColumn:'1/-1' }}>
                ...
                <input style={inp} placeholder="e.g. Aizat Nurlanovna" required value={form.fullName} onChange={e => set('fullName', e.target.value)} />
              </div>
              <div>
                <label style={lbl}>Faculty *</label>
                <select style={inp} required value={form.faculty} onChange={e => set('faculty', e.target.value)}>
                  <option value="">Select faculty…</option>
                  {FACULTIES.slice(1).map(f => <option key={f}>{f}</option>)}
                </select>
              </div>
              <div>
          ...
          <div style={card}>
            ...
            <div style={{ display:'grid', gap:16 }}>
              <div>
                ...
                <input style={inp} placeholder="e.g. Calculus, Python, Data Structures" required value={form.subjects} onChange={e => set('subjects', e.target.value)} />
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                <div>
                  <label style={lbl}>Individual lesson price (₸/hr) *</label>
                  <input style={inp} type="number" min="500" placeholder="e.g. 3000" required value={form.individual} onChange={e => set('individual', e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Group lesson price (₸/hr) *</label>
                  <input style={inp} type="number" min="500" placeholder="e.g. 1800" required value={form.group} onChange={e => set('group', e.target.value)} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ display:'grid', gap:16 }}>
              <div>
                ...
                <textarea style={{ ...inp, resize:'vertical', minHeight:100 }} placeholder="I'm a 3rd-year CS student who loves…" required value={form.bio} onChange={e => set('bio', e.target.value)} />
              </div>
              <div>
            ...
            <div style={{ display:'grid', gap:16 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                <div>
                  ...
                  <input style={inp} placeholder="+7 701 000 0000" required value={form.phone} onChange={e => set('phone', e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Email *</label>
                  <input style={inp} type="email" placeholder="you@cau.kz" required value={form.email} onChange={e => set('email', e.target.value)} />
                </div>
              </div>
            </div>
          </div>
...
function AdminPanel() {
  ...
  const act = (id, status) => setApps(prev => prev.map(a => a.id === id ? {...a, status} : a));
                ...
                <div style={{ padding:'20px 24px', display:'flex', alignItems:'center', gap:18, flexWrap:'wrap' }}>
                  ...
                  <div style={{ flex:1, minWidth:200 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', marginBottom:5 }}>
                      <span style={{ fontWeight:800, fontSize:16 }}>{app.name}</span>
                      <FacultyBadge faculty={app.faculty} />
                      <span style={{ fontSize:12, color:'oklch(0.62 0.04 278)' }}>Year {app.year}</span>
                    </div>
                    <div style={{ fontSize:13, color:'oklch(0.55 0.05 278)', marginBottom:4 }}>
                      {app.subjects.join(' · ')} · <strong>{app.individual.toLocaleString()} ₸</strong> individual · <strong>{app.group.toLocaleString()} ₸</strong> group
                    </div>
