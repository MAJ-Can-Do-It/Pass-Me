...
// ─── FindTutors ───────────────────────────────────────────────────────────────
function FindTutors() {
  const [faculty, setFaculty] = useStateP1('All Schools');
  const [search, setSearch] = useStateP1('');

  const filtered = TUTORS.filter(t => {
    if (faculty !== 'All Schools' && t.school !== faculty) return false;
      ...
      <div className="container" style={{ marginTop:-32 }}>
        ...
        <div style={{
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
            ...
            <button onClick={() => { setFaculty('All Schools'); setSearch(''); }} style={{
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:16 }}>
            {FACULTIES.map(f => {
              ...
              return (
                ...
                }}>{f === 'All Schools' ? 'All' : (FACULTY_STYLES[f]?.label || f)}</button>
              );
            })}
          </div>
        </div>
        ...
        <div style={{ marginBottom:22, color:'oklch(0.55 0.05 278)', fontSize:14, fontWeight:500 }}>
          ...
          {faculty !== 'All Schools' && <span> · <span style={{ color:'oklch(0.48 0.26 278)', fontWeight:600 }}>{faculty}</span></span>}
        </div>
...
function TutorProfile({ tutorId }) {
  ...
  const fs = FACULTY_STYLES[tutor.school] || { color:"oklch(0.48 0.26 278)", bg:"oklch(0.95 0.05 278)", dark:"oklch(0.28 0.28 278)", darkEnd:"oklch(0.38 0.24 292)" };
  const [narrow, setNarrow] = useStateP1(typeof window !== 'undefined' && window.innerWidth < 900);
  useEffectP1(() => {
  ...
  return (
    <div style={{ minHeight:'100vh', paddingBottom:80 }}>
      {/* Header */}
      <div style={{
        background:`linear-gradient(140deg, ${fs.dark || 'oklch(0.28 0.28 278)'} 0%, ${fs.darkEnd || 'oklch(0.38 0.24 292)'} 100%)`,
        padding:'44px 0 68px',
      }}>
        <div className="container">
          <button onClick={() => navigate('find-tutors')} style={{
          ...
          <div style={{ display:'flex', gap:26, alignItems:'flex-start', flexWrap:'wrap' }}>
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:14, flexWrap:'wrap', alignItems:'center', marginBottom:14 }}>
                <span style={{ color:'rgba(255,255,255,0.88)', fontSize:14, fontWeight:600 }}>{tutor.school}</span>
                <span style={{ color:'rgba(255,255,255,0.50)' }}>·</span>
                <span style={{ color:'rgba(255,255,255,0.80)', fontSize:14 }}>{tutor.program}</span>
                <span style={{ color:'rgba(255,255,255,0.50)' }}>·</span>
                <span style={{ color:'rgba(255,255,255,0.80)', fontSize:14 }}>Year {tutor.year}</span>
                <span style={{ color:'oklch(0.92 0.14 82)', fontWeight:700 }}>★ {tutor.rating}</span>
                <span style={{ color:'rgba(255,255,255,0.75)', fontSize:13 }}>{tutor.sessions} sessions</span>
              </div>
      ...
      <div className="container" style={{ marginTop:-24 }}>
        <div style={{ display:'grid', gridTemplateColumns: narrow ? '1fr' : '1fr 340px', gap:22, alignItems:'start' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:'oklch(0.58 0.05 278)', marginTop:2 }}>per hour</div>
                  </div>
                  <div style={{ fontWeight:900, fontSize:20, color:fs.color }}>{tutor.individual.toLocaleString()} UZS</div>
                </div>
                <div style={{
                  padding:'18px 20px', borderRadius:16,
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:'oklch(0.58 0.05 278)', marginTop:2 }}>per student</div>
                  </div>
                  <div style={{ fontWeight:900, fontSize:20, color:'oklch(0.42 0.10 278)' }}>{tutor.group.toLocaleString()} UZS</div>
                </div>
              </div>
            </div>
